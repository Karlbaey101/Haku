...
def delete(ctx, issue_number):
    ...
    # Find the issue file
    matching_files = [f for f in os.listdir(issues_dir) if f.startswith(f"{issue_number}.") and f.endswith('.md')]
...
def pull(ctx):
    ...
    if not ctx.obj.config.repo_url or not ctx.obj.config.token:
        click.echo("Repository not linked or token not set. Use 'haku init' and 'haku token' first.")
        return
